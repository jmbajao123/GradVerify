...
$department_name = $d_id= $course_status = "";
$c_id = 0; // Initialize department ID

// Check if 'd_id' is provided in the URL to fetch department details
if (isset($_GET['c_id'])) {
    $c_id = intval($_GET['d_id']); // Sanitize input to prevent SQL injection

    // Fetch department details
    $query = "SELECT c_id, d_id, course_status FROM course WHERE c_id = ?";
    if ($stmt = $conn->prepare($query)) {
        $stmt->bind_param("i", $c_id); // Bind the d_id parameter
        $stmt->execute();
