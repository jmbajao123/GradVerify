...
<head>
  ...
  <title>Course List Reports - Credentials Verification Sibugay Technical Institute Inc.</title>
      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-body">
            <h2 class="section-title">Course List Reports
                <a href="c_print.php" style="float: right;" target="_blank" class="btn btn-outline-primary">Generate</a>
            </h2>
...
$total_sql = "SELECT COUNT(*) AS total FROM course";
$total_result = mysqli_query($conn, $total_sql);
$total_row = mysqli_fetch_assoc($total_result);
$total_records = $total_row['total'];
$total_pages = ceil($total_records / $limit);

// Fetch data with department name using JOIN
$sql = "SELECT c.c_id, c.course_name, c.course_status, d.department_name 
        FROM course c
        JOIN departments d ON c.d_id = d.d_id
            ...
            <table class="table table-striped table-md">
                ...
                <?php if (mysqli_num_rows($result) > 0): 
                    ...
                    while ($row = mysqli_fetch_assoc($result)): 
                        $status_badge = ($row['course_status'] == 'Active') ? 'badge-success' : 'badge-danger';
                ?>
                <tr>
                    <td><?= $count; ?></td>
                    <td><?= htmlspecialchars($row['department_name']); ?></td> <!-- Display department name -->
                    <td><?= htmlspecialchars($row['course_name']); ?></td>
                    <td><div class="badge <?= $status_badge; ?>"><?= htmlspecialchars($row['course_status']); ?></div></td>
                </tr>
                <?php 
                    $count++;
                    endwhile; 
		       ...
		       <form method="post" action="course.php">
		       	<div class="col-lg-12">
		       		...
		       		<select class="form-control" name="d_id" id="d_id">
					    ...
					    $sql = "SELECT d_id, department_name FROM departments WHERE department_status = 'Active'";
					    $result = mysqli_query($conn, $sql);

					    if (mysqli_num_rows($result) > 0) {
					        while ($row = mysqli_fetch_assoc($result)) {
					            echo "<option value='{$row['d_id']}'>" . htmlspecialchars($row['department_name']) . "</option>";
					        }
					    }
		       	...
		       	<div class="col-lg-12">
		       		...
		       		<input type="text" name="course_name" id="course_name" class="form-control" placeholder="Input the Course Name" required>
		       	</div>
		       	<div class="col-lg-12">
		       		<br>
		       	</div>
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        echo "<script>
                Swal.fire({
                    ...
                    window.location.href = 'course.php'; // Redirect to courses page
                });
              </script>";
    } else {
        // Insert new course
